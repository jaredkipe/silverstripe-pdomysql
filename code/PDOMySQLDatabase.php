...
class PDOMySQLDatabase extends SS_Database {
	 ...
	 *	- port: The database tcp port (note that you should not use 'localhost' as host with a port)
	 *  - timezone: (optional) The timezone offset. For example: +12:00, "Pacific/Auckland", or "SYSTEM"
	 */
	public function __construct($parameters) {
		$dsn_base = "mysql:host={$parameters['server']}";
		if(!empty($parameters['port'])) {
			$dsn_base .= ";port={$parameters['port']}";
		}
		
		if(Config::inst()->get('PDOMySQLDatabase', 'connection_charset')) {
			$options = array(
		    	PDO::MYSQL_ATTR_INIT_COMMAND => 'SET NAMES ' . Config::inst()->get('PDOMySQLDatabase', 'connection_charset'),
			);
		} else {
			$options = array(
		...
		try {
			$dsn = $dsn_base . ";dbname={$parameters['database']}";
			$this->dbConn = new PDO($dsn, $parameters['username'], $parameters['password'], $options);
			$this->dbConn->query("SET sql_mode = 'ANSI'");
			$this->_finishDb($parameters);
		} catch (PDOException $e) {
			// fall through to attempt to create database after install
			try {
				$this->dbConn = new PDO($dsn_base, $parameters['username'], $parameters['password'], $options);
				$this->dbConn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
				$this->dbConn->query("SET sql_mode = 'ANSI'");
				$this->dbConn->query("CREATE DATABASE \"{$parameters['database']}\"");
				$this->dbConn->query("USE \"{$parameters['database']}\"");
				$this->_finishDb($parameters);
			} catch (PDOException $e) {
				$this->databaseError("Couldn't connect to MySQL database | " . $e->getMessage());
			}
		}	
	}
	
	protected function _finishDb($parameters) {
		$this->active = $this->dbConn;
	...
	public function getLastError() {
		if ($this->dbConn) {
			return 'PDO MySQL Error: ' . print_r($this->dbConn->errorInfo(), true);
		} else {
			return 'PDO MySQL Error';
		}
	}
	...
	public function query($sql, $errorLevel = E_USER_ERROR) {
		...
		if ($handle) {
			$this->lastAffectedRows = $handle->rowCount();
		}
		...
		if (!$handle && $errorLevel) {
			$this->databaseError('Couldn\'t run query: $sql - ' . print_r($this->dbConn->errorInfo(),true), $errorLevel);
		}
		return new PDOMySQLQuery($this, $handle);
	}
	...
	public function createTable($table, $fields = null, $indexes = null, $options = null, $advancedOptions = null) {
				//throw new Exception('inside createTable');
	...
	public function addslashes($value){
		return trim($this->dbConn->quote($value), '\'');
	}
	...
	/**
	 *	Though PDO supports transactions natively, savepoints do not seem to be supported...
	 */
	/*
 ...
 * @subpackage model
 */
class PDOMySQLQuery extends SS_Query {
	/**
	 * The PDOMySQLDatabase object that created this result set.
	 * @var PDOMySQLDatabase
	 */
	protected $database;

	/**
	 * The PDOStatement used to handle data.
	 * @var PDOStatement
	 */
	protected $handle;
	...
	public function __destruct() {
		if(is_object($this->handle)) $this->handle->closeCursor();
	}
	
	public function seek($row) {
		if(is_object($this->handle)) $this->handle->fetch(PDO::FETCH_ASSOC, PDO::FETCH_ORI_ABS, $row);

	}
	
	public function numRecords() {
		if(is_object($this->handle)) return $this->handle->rowCount();
	}

	public function nextRecord() {
		if(is_object($this->handle) && ($data = $this->handle->fetch(PDO::FETCH_ASSOC))) {
			return $data;
		} else {
			return false;
		}
	...
	//	return $this->handle->fetchColumn($column);
	//}
}